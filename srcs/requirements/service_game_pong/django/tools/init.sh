...
sed -i "s/\${DOMAIN_NAME}/$DOMAIN_NAME/g" /django_web_app/transcendence/transcendence/settings.py
...
source /django_web_app/env/bin/activate \
	...
	&& (
        python3 -c "import os; os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'transcendence.settings'); import django; django.setup(); from django.contrib.auth.models import User; exit(0 if User.objects.filter(username='$SUPER_USER_NAME').exists() else 1)" \
        || \
